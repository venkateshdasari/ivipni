...
export class CategoryProductDetailsPage implements OnInit {
  ...
  constructor(
    ...
    var category_value;
    category_value = _this.navParams.get("sub_category_id");
    _this.categoryname = _this.navParams.get("sub_category_name");
    _this.logger.debug(
      "cheking the id of the get navParam in catprod details " +
      _this.navParams.get("sub_category_id")
    );

    _this.promotionalProducts = _this.navParams.get("category_id");
    _this.bestSellerProducts = _this.navParams.get("category_id");

    if (category_value) {
      _this.logger.debug("category-value" + category_value);
      this.category_id = category_value;
            ...
            for (var j = 0; j < _this.catprodArray[i].option1[q].option2.length; j++) {
              ...
              if (_this.dataoption) {
                if (_this.dataoption.quantity > 0) {
                  ...
                  k++;
                  console.log(_this.prods[i].option2value);
                }
              }
