...
_this.logger.debug("allTotal " + JSON.stringify(_this.allTotal));
  ...
  myValueChange(ev, prod, op, qa) {
    ...
    console.log("ev " + JSON.stringify(ev)+"prod " + JSON.stringify(prod)+"op " + JSON.stringify(op)+"qa" +qa);
...
loading.present();  
      ...
      this.users.updateCart(addProduct, function(result, data) {
        if (result == "1") {
          ...
          console.log("allTotal " + JSON.stringify(_this.allTotal));
        loading.dismiss();
          _this.presentToast();
...
loading.present();  
  ...
  checkoutNav() {
    ...
    console.log("proceed " + _this.proceed);
    if (_this.checkout) {
      if (_this.proceed) {
     ...
     if(this.quantityDebug.length > 1){
      ...
      for(var j=0;j < _this.quantityDebug.length;j++){
          if(item.cart_id != _this.quantityDebug[j].cart_id){
            ...
            _this.debug = {cart_id:_this.quantityDebug[j].cart_id,
