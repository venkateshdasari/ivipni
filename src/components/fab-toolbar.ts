...
export class FabToolbar {
    ...
    constructor(public el: ElementRef, public renderer: Renderer,public sms:SMS ) {}
    ...
    //   {icon: 'logo-whatsapp', title: 'Whatsapp', color: 'primary', handler: ()=> {
    ...
    //             },
    //             ()=>{
    //                 alert("please install whats up")
    //             })
    //     }},
     {icon: 'call', title: 'Call', color: 'primary', handler: ()=> {
            //console.log('close me')

            //reffer
            //http://pointdeveloper.com/ionic-2-beginner-dial-a-number-in-ionic-2-apps/
            window.location = 'tel:8489949143';
        }},

      {icon: 'text', title: 'Text', color: 'primary', handler: ()=> {
          //console.log('dont close me'); return false

          //check this website to reffer
          //http://pointdeveloper.com/ionic-2-beginner-send-sms-ionic-2-cordova-sms-plugin/
            var options={
        ...
        this.sms.send('8489949143', 'Hello world!',options)
                .then(()=>{
                    alert("success");
                },()=>{
                    alert("failed");
    ...
    public get width() {
        console.log("invoking get width() Fn");
        let width = window.innerWidth / 56 * 2;
        return 'scale(' + width + ')';
    }

    public openButton(): void {
        console.log("invoking openButton() Fn");
    ...
    public closeButton(): void {
        console.log("invoking closeButton Fn");
        this.renderer.setElementClass(this.el.nativeElement, 'activated', !this.active);
        setTimeout(() => {
